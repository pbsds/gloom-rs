...
unsafe fn create_vao(vertices: &Vec<f32>, indices: &Vec<u32>) -> u32 {
    ...
    let index: u32 = 0;
    let dim: i32 = 3;
    let stride: i32 = dim * std::mem::size_of::<f32>() as i32;

    gl::VertexAttribPointer(index, dim, gl::FLOAT, gl::FALSE, stride, 0 as *const c_void);
    ...
    // * Return the ID of the VAO                gl::DrawElements(gl::TRIANGLES, 3, gl::UNSIGNED_INT, 0 as *const c_void);
...
fn main() {
    ...
    let render_thread = thread::spawn(move || {
        ...
        let mut shader: shader::Shader = unsafe {
            let mut shader = shader::ShaderBuilder::new()
                .attach_file("./shaders/simple.vert")
                .attach_file("./shaders/simple.frag")
                .link();

            shader.activate();
            shader.rotate(std::f32::consts::PI / 4.0, util::Direction::Z); // enable for rotation
            shader.invert();
            shader
        };
        ...
        let mut color_vec: glm::Vec4 = glm::vec4(1.0, 1.0, 1.0, 1.0);
        loop {
...
unsafe fn draw_circle(indices: &Vec<u32>) {
    gl::DrawElements(
        gl::LINE_LOOP,
        indices.len() as i32,
        gl::UNSIGNED_INT,
